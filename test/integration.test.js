...
describe('Path Resolution', () => {
    ...
    it('should sanitize branch names with slashes', async () => {
        ...
        const result = await runCli(['new', 'feature/test-branch', '--path', worktreePath, '--editor', 'none'], ctx.repoDir);
...
describe('wt new', () => {
    ...
    it('should create a new worktree for a new branch', async () => {
        ...
        const result = await runCli(['new', 'feature/new-feature', '--path', worktreePath, '--editor', 'none'], ctx.repoDir);
    ...
    it('should reuse existing worktree directory', async () => {
        ...
        await runCli(['new', 'feature/existing', '--path', worktreePath, '--editor', 'none'], ctx.repoDir);
        // Try to create again with same path
        const result = await runCli(['new', 'feature/existing', '--path', worktreePath, '--editor', 'none'], ctx.repoDir);
        expect(result.exitCode).toBe(0);
...
describe('wt remove', () => {
    ...
    it('should remove an existing worktree by path', async () => {
        ...
        // Create worktree
        await runCli(['new', 'feature/to-remove', '--path', worktreePath, '--editor', 'none'], ctx.repoDir);
        // Remove it with force flag (to skip confirmation)
        const result = await runCli(['remove', worktreePath, '--force'], ctx.repoDir);
        expect(result.exitCode).toBe(0);
...
describe('Atomic Operations', () => {
    ...
    it('should rollback on failure', async () => {
        ...
        const result = await runCli(['new', 'feature/rollback', '--path', worktreePath, '--editor', 'none', '--install', 'nonexistent-package-manager-xyz'], ctx.repoDir);
...
describe('Branch Name Validation', () => {
    ...
    it('should reject invalid branch names', async () => {
        ...
        for (const name of invalidNames) {
            const result = await runCli(['new', name, '--editor', 'none'], ctx.repoDir);
            expect(result.exitCode).toBe(1);
        }
    });
});
