...
/**
 * Options for the worktree workflow
 */
export interface WorkflowOptions {
    /** Custom editor to use instead of configured default */
    editor?: string;
    /** Skip the dirty state check entirely */
    skipDirtyCheck?: boolean;
    /** Custom message for the dirty state prompt */
    dirtyStateMessage?: string;
    /** Stash message prefix */
    stashPrefix?: string;
}
...
export interface WorkflowContext {
    /** Whether the main repo is bare */
    isBare: boolean;
    /** Hash of stashed changes (if any) */
    stashHash: string | null;
    /** Function to manually trigger stash restoration */
...
export interface WorkflowResult<T> {
    /** The result from the main operation */
    result: T;
 ...
 * });
 */
export async function executeWorkflow<T>(config: {
    /** The main operation to perform, receives context */
    operation: (context: WorkflowContext) => Promise<T>;
    /** Function to get the path to open in editor (receives operation result) */
    getEditorPath?: (result: T) => string | null;
    /** Whether to open the result in an editor */
    ...
    const {
        ...
        dirtyStateMessage = "Your main worktree has uncommitted changes.",
        stashPrefix = "wt-workflow",
    } = options;
 ...
 */
export async function checkWorktreeStatus(path: string): Promise<{
    exists: boolean;
    isWorktree: boolean;
}> {
    let exists = false;
