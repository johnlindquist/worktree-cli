...
import { getWorktrees, WorktreeInfo } from "../utils/git.js";
...
export async function purgeWorktreesHandler() {
    try {
        ...
        for (const wt of purgeWorktrees) {
            ...
            if (wt.prunable) status += chalk.yellow(' [prunable]');
        ...
        for (const wt of selectedWorktrees) {
            ...
            try {
                ...
                const execaError = removeError as ExecaError;
                ...
                if (stderr.includes("modified or untracked files")) {
                    ...
                    if (forceAnswer) {
                        try {
                            await withSpinner(
                                `Force removing worktree for branch "${wt.branch || '(detached)'}"...`,
                                async () => {
