 ...
 */
async function fetchGitHubPRBranch(prNumber) {
    const token = process.env.GITHUB_TOKEN;
    if (!token) {
        throw new Error("GITHUB_TOKEN environment variable is required when 'gh' CLI is not installed.\n" +
 ...
 */
async function fetchGitLabMRBranch(prNumber) {
    const token = process.env.GITLAB_TOKEN;
    if (!token) {
        throw new Error("GITLAB_TOKEN environment variable is required when 'glab' CLI is not installed.\n" +
    ...
    try {
        ...
        if (error.code === 'ENOENT' || error.message?.includes("ENOENT")) {
            ...
            try {
                ...
                throw new Error(`Failed to fetch ${requestType} via API: ${apiError.message}\n` +
