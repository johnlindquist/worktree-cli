...
export class AtomicWorktreeOperation {
     ...
     */
    async createWorktree(path, branch, createBranch = false) {
     ...
     */
    async createWorktreeFromRemote(path, branch, remoteBranch) {
     ...
     */
    async runInstall(packageManager, cwd) {
 ...
 */
export async function withAtomicRollback(operation) {
    const atomic = new AtomicWorktreeOperation();
    try {
